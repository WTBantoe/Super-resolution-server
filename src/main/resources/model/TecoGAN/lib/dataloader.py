...
def loadHR(FLAGS, tar_size):
    ...
    with tf.device('/cpu:0'):
        ...
        with tf.variable_scope('load_frame'):
            ...
            for fi in range(FLAGS.RNN_N):
                HR_data = tf.image.convert_image_dtype(tf.image.decode_png(tf.read_file(output[fi]), channels=3),
                                                       dtype=tf.float32)
                if (FLAGS.movingFirstFrame):
                    if (fi == 0):
                    ...
                    HR_data = tf.cond(moving_decision < 0.7, lambda: tf.identity(HR_data),
